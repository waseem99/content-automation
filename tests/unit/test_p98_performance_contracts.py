...
def test_experiment_allows_delivery_variants_of_same_release() -> None:
    ...
    assert "attname='experiment_id'" in flexibility
    assert "attname='final_release_id'" in flexibility
    assert "conkey=ARRAY[experiment_attnum,release_attnum]::smallint[]" in flexibility

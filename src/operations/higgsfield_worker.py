...
class HiggsfieldManagedWorker:
    """Claim only approved Higgsfield premium clips; never approve or publish."""
    ...
    def __init__(self, database: Database) -> None:
        ...
        self.adapter = HiggsfieldCliRendererAdapter()
        self.worker_id = os.getenv("HIGGSFIELD_WORKER_OPERATOR_ID", "higgsfield-worker")
        self.artifact_root = Path(
            os.getenv("LOCAL_ARTIFACT_ROOT", ".runtime/artifacts")
        ).expanduser().resolve()
        self.artifact_root.mkdir(parents=True, exist_ok=True)
        self.lease_seconds = int(os.getenv("HIGGSFIELD_WORKER_LEASE_SECONDS", "1800"))
    ...
    def run_once(self) -> dict[str, Any]:
        ...
        try:
            ...
            if provider_job_id:
                state = self.adapter.poll(provider_job_id)
            else:
            ...
            if not state.terminal:
                state = self.adapter.wait(
                    ...
                    poll_seconds=float(os.getenv("HIGGSFIELD_POLL_SECONDS", "5")),
                )
            ...
            if state.status != "succeeded":
                raise HiggsfieldCliError(
                    ...
                    state.error or "The Higgsfield generation failed.",
                    retryable=False,
                )

            output_dir = self.artifact_root / "managed" / str(job["id"])
            ...
            retryable = isinstance(exc, HiggsfieldCliError) and exc.retryable
            error_code = exc.code if isinstance(exc, HiggsfieldCliError) else "higgsfield_worker_execution_failed"
            self.jobs.fail(
                GenerationJobFailure(
                    job_id=job["id"],
                    attempt_id=attempt["id"],
    ...
    def _input_paths(self, request_payload: dict[str, Any]) -> list[Path]:
        ...
        with self.database.connection() as conn:
            for asset_id in asset_ids:
                ...
                if not asset:
                    raise HiggsfieldCliError(
                        "higgsfield_input_asset_missing",
    ...
    def _register_output(
        ...
        *,
        ...
        routing_item_id = str(
            request_payload.get("request_metadata", {}).get("routing_item_id")
            or request_payload.get("routing_item_id")
            or job["id"]
        )
        artifact = self.shared.create_artifact_version(
            request=ArtifactVersionRequest(
                ...
                artifact_key=f"managed/{job['portfolio_content_id']}/{routing_item_id}",
                ...
                metadata={
                    ...
                    "selected_candidate_id": request_payload.get("request_metadata", {}).get(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the official Higgsfield managed-render worker")
    ...
    try:
        worker = HiggsfieldManagedWorker(database)
        while True:

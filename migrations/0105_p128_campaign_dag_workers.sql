...
CREATE TABLE football_brief.campaign_task_graphs (
    ...
    parent_graph_id uuid REFERENCES football_brief.campaign_task_graphs(id) ON DELETE RESTRICT,
    ...
    task_schema_version text NOT NULL DEFAULT 'p128-v1'
        CHECK (length(btrim(task_schema_version)) BETWEEN 3 AND 80),
    ...
    CHECK (version=1 OR parent_graph_id IS NOT NULL),
    CHECK (version<>1 OR parent_graph_id IS NULL),
...
CREATE TABLE football_brief.campaign_task_workers (
    ...
    current_generation text NOT NULL DEFAULT 'p128-v1'
        CHECK (length(btrim(current_generation)) BETWEEN 3 AND 80),
    ...
    CHECK (cardinality(capabilities)=cardinality(ARRAY(SELECT DISTINCT unnest(capabilities))))
);
...
BEGIN
    IF TG_OP='DELETE' THEN
        RAISE EXCEPTION 'Campaign tasks are immutable and cannot be deleted';
    END IF;
...
BEGIN
    IF TG_OP='DELETE' THEN
        RAISE EXCEPTION 'Campaign task attempts cannot be deleted';
    END IF;
    IF OLD.status<>'running' THEN
        RAISE EXCEPTION 'Terminal campaign task attempts are immutable';
    END IF;
...
COMMENT ON TABLE football_brief.campaign_tasks IS
    'Durable campaign DAG tasks with capability requirements, fair SKIP LOCKED claims and immutable terminal evidence.';
COMMENT ON TABLE football_brief.dag_acceptance_runs IS
    'Retained 100-worker and million-task orchestration acceptance evidence; no renderer throughput is implied.';

...
def main() -> int:
    ...
    requests = build_keyframe_requests(
        pilots_root, artifact_root, pilot_id=args.pilot,
        shot_ids={item.strip() for item in args.shots.split(",") if item.strip()} if args.shots else None,
    )
    if args.command == "plan":
        print(json.dumps({
            "schema_version": "p68.keyframe_generation_plan.v1",
            "request_count": len(requests),
            "requests": [{"pilot_id": item.pilot_id, "shot_id": item.shot_id, "seed": item.seed, "idempotency_key": item.idempotency_key} for item in requests],
            "provider_calls_made": 0, "vercel_deployment_required": False, "publish_allowed": False,
        }, indent=2))
        return 0
    ...
    provider = ComfyUIKeyframeProvider(
        base_url=base_url, workflow_path=Path(args.workflow), checkpoint=checkpoint,
    ...
    controller = KeyframeGenerationController(
        pilots_root=pilots_root, artifact_root=artifact_root, provider=provider,
        checkpoint=checkpoint, license_type=license_type, license_url=license_url,

...
| Phase 1 platform foundation | Previously deployed migrations cannot be silently edited | automated | `tests/acceptance/test_migration_immutability.py` | Manifest and applied-checksum tests catch edits. |
...
| Phase 1 platform foundation | Worker execution is idempotent under duplicate requests | automated | `tests/integration/test_worker_execution.py`<br>`tests/integration/test_idempotency_concurrency.py` | Duplicate and concurrent requests do not create duplicate billable rows. |

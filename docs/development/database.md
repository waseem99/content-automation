...
Set the development connection in `.env` using the local credentials configured for your machine:

```dotenv
DATABASE_URL=postgresql://football_brief:<local-password>@localhost:5432/football_brief_dev
...
Never point `FOOTBALL_BRIEF_TEST_DATABASE_URL` at a shared, staging, or production database.
